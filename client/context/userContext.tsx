...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 401) {
          // Unauthorized: Check if it's due to invalid email or password
  ...
  const addUsername = async (username: string) => {
    try {
      ...
      // const customSession = user as CustomSession;
      // const userId = customSession?.id;

      // if (!userId) {
      //   throw new Error("User not logged in");
      // }

      const response = await fetch(`http://localhost:3000/api/add-username`, {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      console.error("Error during addUsername:", error.message);
      throw error;
    }
  };
