...
export default function LoggedInScreen({ navigation }: Props) {
  const theme = useTheme();
  const { user, getDueDate } = useAuth(); // Access user information from context
  ...
  return (
    ...
    >
      {user && user.babies && user.babies.length > 0 && (
        <>
          ...
          <Text style={styles.text}>
            ...
            {user.babies &&
              ...
              user.babies[0].dueDate &&
              new Date(user.babies[0].dueDate).toLocaleDateString()}
          </Text>
        </>
      )}
